    ...
    async findOne(userId: string): Promise<StudentProfileResponse> {
        try {
            const student = await this.studentRepository
                ...
                .leftJoin(ProjectEntity, 'bonusProject', 'bonusProject.user_id = :userId AND bonusProject.type =' +
                    ' :typeB', {typeB: projectTypeEnum.bonusProject})
                .leftJoin(ProjectEntity, 'portfolioProject', 'portfolioProject.user_id = :userId AND' +
                    ' portfolioProject.type = :typeP', {typeP: projectTypeEnum.portfolio})
                .select([
                    'user.email',
                    'student.phoneNumber',
                    'student.firstName',
        ...
        try {
            ...
            if (!Object.values({...restOfDetails, githubName}).every(detail => detail === undefined)) {
                const updateResults = await Promise.all([
                    ...
                    this.projectService.updateProject(student.userId, bonusProjectUrl, projectTypeEnum.bonusProject),
                    this.projectService.updateProject(student.userId, portfolioUrl, projectTypeEnum.portfolio),
                ])

               updateResults.forEach((el) => updates.push(el as UpdateResult));
            }

            const isSuccess = updates.some(updateResult => updateResult.affected > 0);
